...
import pandas as pd
import pydicom
from ML.classification.feature_extraction.extract_radiomics import save_radiomic_features
    ...
    for root, dirs, files in os.walk(dynamicrenal):
                for filename in files:
                    if (base_name in filename) and filename.endswith(".dcm") and ("POST2" not in filename):
                        ...
                        if pydicom.dcmread(file_path).pixel_array.shape[0] > 100 and pydicom.dcmread(file_path).pixel_array.shape[0] <= 180:
...
if __name__ == "__main__":
            ...
            if filename.endswith(".json"):
                ...
                if time_series_path:
                    ...
                    metadata_list.append({
                        ...
                        "TimeSeriesPath": os.path.abspath(time_series_path)
                    })
    ...
    save_radiomic_features(df_metadata)
